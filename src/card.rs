...
/// The *standard* side deck: every opponent's baseline pool. Not the player's
/// starter, and not what Quick Play deals — since spec 024 every match deals
/// the player's built deck; a fresh profile builds from
/// `profile::STARTER_SIDE_DECK`, which is Outer-tier only. One tunable
/// constant.
...
/// player uses their built deck in every match since spec 024; an opponent
/// uses its profile's deck, built from [`DEFAULT_SIDE_DECK`]).
...
mod tests {
    ...
    fn all_side_cards_cover_the_default_deck() {
        ...
        // standard deck contains (the opponent baseline), or the deck-builder
        // couldn't represent it. The starter deck's own universe membership is
        // pinned in `profile.rs`.
