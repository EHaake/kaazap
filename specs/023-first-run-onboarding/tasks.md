...
|---|---|---|---|
| **Experiment 1, spec 3** — session `claude-fable-5-1` at medium throughout. Fable allowance at start: not recorded (the opening prompt's placeholder was left unfilled); after Phase 1 and the experiment 2 setup: 26% left (person's reading, 2026-09-14); at spec end: (person's reading, date). | — | — | header |
| **Experiment 2** — from the next implementer dispatch (T006 on), the implementer is `sdd-implementer-fable` (`claude-fable-5-1` at medium); `sdd-implementer` (opus) is the fallback. Started 2026-09-13. Fable allowance at start of experiment 2: 26% left (person's reading, 2026-09-14). | — | — | header |
