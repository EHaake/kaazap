...
| **Experiment 2** — from the next implementer dispatch (T006 on), the implementer is `sdd-implementer-fable` (`claude-fable-5-1` at medium); `sdd-implementer` (opus) is the fallback. Started 2026-09-13. Fable allowance at start of experiment 2: (person's reading pending — not supplied with the instruction). | — | — | header |
