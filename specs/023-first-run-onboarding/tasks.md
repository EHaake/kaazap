...
|---|---|---|---|
| **Experiment 1, spec 3** — session `claude-fable-5-1` at medium throughout. Fable allowance at start: not recorded (the opening prompt's placeholder was left unfilled); after Phase 1 and the experiment 2 setup: 26% left (person's reading, 2026-09-14); at spec end: 20% left (person's reading, 2026-09-15, after the merge). Draw across the spec is not computable — no start reading; from the 2026-09-14 reading (26% left, after Phase 1 and the experiment 2 setup) to spec end the draw was 6 points, with every implementer and reviewer dispatch at opus and only the session itself on fable. | — | — | header |
