...
| T009 close-out (sdd-implementer) | opus → opus (economy: the close-out row names `sdd-implementer`, no override) | 134K | 1 | yes | — | `closeout-main-docs.md`, 961 lines: ROADMAP two edits (no backlog entry existed to close), DECISIONS append, not-drafted-here, mechanical checks, 17 ACs with evidence. Warning counts compared via a throwaway `git worktree` of `main` with separate `CARGO_TARGET_DIR`s — 0 and 0, no branch switched. **Finding: `spec.md`'s own Summary and Goal 1 say "however kaazap ends"**, the phrase the task forbids the close-out from using, because a signal still leaves the terminal unrestored; only the Key behavior section enumerates. The close-out follows the plan and says so |
| Pre-merge sweep (skeptical-reviewer) | opus → opus | 158K | 1 | — | **1 blocking** | B1: `plan.md` §Design 7 still carried the notice wording T008 correctly replaced, so the branch would have merged with a plan asserting behaviour the system doesn't have. Plus S2 (`spec.md` carries the same false claim — record it, don't edit the person's document), S3 (drifted line citations in the close-out), S4 (two wrong counts of `app.rs` tests, one of them in text that lands on `main`), S5 (stale diff stat). Confirmed all 17 ACs trace to real evidence, re-derived the notice's fit arithmetic and the key ladder independently, and verified the close-out holds the *endings `spec.md` enumerates* wording everywhere including the ROADMAP entry |
| Sweep fixes (sdd-implementer) | opus → opus | 47K | 1 | yes | — | Documents only, no code. `plan.md` amended in the T008a style; the DECISIONS draft now names both carriers of the false claim; AC 6 cites assertion text; §1b pins no count; §4's stat re-run at HEAD with a sentence saying why a stat is always one commit behind |
| Sweep re-review (skeptical-reviewer) | opus → opus | 30K | 1 | — | 0 blocking | **All five closed. Signed off: mergeable.** One note for the log: the roadmap bullet's inherited "seven `app.rs` unit tests" is pre-existing chore text neither pass re-derived — worth checking when that follow-up is actually done, not now |
| **Economy profile — the comparison the constitution asks for** | — | — | — | — | — | **Planning 390K** (planner 225K + sign-off 126K + planner notes 39K) against spec 027's ≈651K on fable — the top tier moving from fable to opus did **not** cost more here; it cost about 40% less, on a spec of comparable size. **Implementer 891K over 16 dispatches** (≈56K each) against 027's ≈504K over 8 (≈63K each): per-dispatch slightly cheaper, but this spec ran **16 dispatches where the plan wrote 9**, because seven sub-lettered tasks (T002a, T003a, T004a, T005a, T007a, T008a, sweep fixes) came out of reviews. **Reviewer 553K over 9 invocations** from Phase 1 on, against 027's ≈486K. **Total ≈1.83M.** Read that against what the sub-lettered tasks bought: T002a fixed a garbled crash report the walkthrough caught 12/12; T003a and T004a turned two vacuous tests into ones that bite; T005a pinned the one new decision in the spec; T008a gave a review finding a durable form. **Tier misses: none.** No task was redone by hand, no escape hatch fired, and every implementer dispatch verified green on its first try |
