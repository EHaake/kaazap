...
| Phase 1 driver walkthrough (orchestrator, scratch profiles, 139×31; real profile/settings/save backed up to scratchpad, checksummed) | — | — | Fresh profile: three groups, headings `Mid Rim  ·  reach the Mid Rim to unlock` / `Core  ·  reach the Core to unlock`, prices and owned counts on every row, cursor on `+1`; ↑ wraps to `±1`, six ↓ from `+1` land on `±1` (never into Mid); two buys of `+1` took 50 → 10 credits (owned ×4 → ×6), a third Enter refused at spendable 0 with nothing changed. Mid profile (Cinder, Scree beaten): Mid heading bare, ↑ wraps to `3&6`, Core still locked with owned ×1 showing. Core profile: all headings bare, ↑ wraps to `±1T`. Core profile → Start Campaign → New Campaign → Yes: `beaten` empty, credits 500 kept; Outfitter shows Mid and Core locked again with `+4`/`±6`/`±1T` owned ×1 intact, ↑ wraps to `±1`. Nothing clips (list rows 6–25, hint 27). **Not verifiable by the driver:** dim/bold attributes (the snapshot is text only) — the person checks the dimming. **Layout finding:** the list block starts at column 48 (headings at 51) while the title centers on column 69; a row is 27 columns (28 with a two-digit owned count), so unlocked rows span 48–74/75 and the row block centers on column 62 — about **7** columns left of the title and balance. (`list_left` sizes the block on the widest line it can ever draw, the locked Mid Rim heading at 42 columns, so the offset is most visible on the Core profile, where no long heading is on screen.) Real data restored twice (after each driver script); SHA-256 of profile.json, settings.json, saves/savegame.json match the pre-driver sums (81bb34d1…, 6fbd0816…, 59866b15…) |
...
| Pre-merge sweep (skeptical-reviewer) | opus | ~125K (measured return; reviewer's own ~80K in / 6K out) | 1 blocking, 8 notes — B1 `tasks.md`/close-out said the session ran on `claude-opus-5` while `CLAUDE.md`'s Fallback names `claude-opus-4-8` and the close-out claimed no amendment needed; orchestrator amended `CLAUDE.md` on `main` in its own commit (3777553: Fallback now points at the session-fallback model the person names, History records spec 025 running entirely on it) and corrected the roadmap's 15-row figure to 21 (6e5db0d, N7). N1/N2/N3/N4/N5/N6 text fixes dispatched; N8 = this row. Reviewer's checks confirmed the fit arithmetic, diff scope, no stale wording, and the "clarified not superseded" call on spec 012 |
| Sweep fixes (sdd-implementer, T003's context) | opus | ~114K cumulative for the T003 dispatch (~18K delta this turn; implementer's own ~86K total) | B1 close-out §3 corrected to name the amendment commit; N1 attestation wording ("attested the screen looked good", not "eyeballed"); N3 AC 7 evidence (heading takes tier/depth as parameters); N4 layout figures re-derived from `row_text`/`list_left` — rows 27–28 wide, span 48–74/75, block centre 62 vs title 69, **about 7 columns left** (the driver row's hand estimate was off by ~4 columns); N5 line cite 248–251; N6 supersession sentence tightened; N2 dimming evidence names what the person saw. Anchors re-verified at main 6e5db0d: 11/11 verbatim and unique |
| Sweep re-review (skeptical-reviewer) | opus | ~39K (measured return; reviewer's own ~36K in / 2.5K out) | B1 half-resolved — B1-a the close-out's header still denied the amendment its own §3 records; B1-b `CLAUDE.md`'s *Tiers by name* still named `claude-opus-4-8` while the amended Fallback clause named `claude-opus-5` and spelled IDs where that list forbids it. Loop cap reached, so the orchestrator applied both: *Tiers by name* now carries the fallback session model and the clause refers to it by role (`main` 5b6197a, own commit); the close-out header names the amendment. N1 acted on too — the AC 2 evidence overstated the attestation; it now records exactly what the person was asked and replied. N5/N7/N4/N3/N6 confirmed correct |
| Orchestrator miss (git) | — | — | A chained shell command failed mid-way (a stale anchor in an inline python heredoc), and the `git stash` / `git checkout main` / `git stash pop` steps that followed committed two spec-025 files to `main` (64b72da). Reverted on `main` with a message explaining it (e92a66f, no force-push), the stash dropped, and the two edits redone on the branch. Lesson for the next spec: never chain a file edit, a branch switch and a commit in one command — edit, verify, then commit as separate steps |
