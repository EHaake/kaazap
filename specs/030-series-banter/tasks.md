...
- **Phase 2 review notes:** (a) AC 4 rests on the board's own rule in `match_series` — the in-flight node equals the locked series — so the sweep should confirm `in_progress` can never point at the locked series during Quick Play (three new callers rely on it: `series_state_now`, `tick`'s `before`, the draw arm). (b) No test pins non-final voices' `all_square` as empty (unreachable, harmless). (c) Lines flagged for the person's edit pass: Greeb's series won/lost mirror each other and echo his match-win lines; Dax repeats the warming-up excuse; Toran's "boy" (he says "lad"); Vessa's "All in, right now."; Sovereign's "Balance. Brief."; near-echoes across voices ("Temporary deviation."/"A temporary debt.", "Your guard weakens."/"Your case weakens.").
- **Phase 3 review notes:** (a) plan §Open questions 4 — the run-over notice covers the venue line fully at 89 columns and clips its first character at 139, yet the line is spoken with its burbles and the run then resets; put to the person at the Phase 3 pause (recommended fix: in `arrive_at_campaign`, when `self.profile.is_broke()`, set `self.speech = None` and return — the `None` is required so the match's closing line doesn't keep burbling on the venue). (b) At 89 the wager modal covers the line row; a line still revealing burbles under it for under a second. (c) "No burble on return" rests on the Back arms calling `open_campaign_home` and settling being silent — the person's ear is the check. (d) `banter_last` isn't persisted, so the first venue line after a relaunch can repeat the pre-quit one (1 in 3) — not a regression. (e) The acknowledgement's MenuSelect and the venue's first burble start on the same tick. (f) `the_venue_rows_breathe_only_around_the_action_row` still draws with `None` (geometry holds; optional hardening).
- **Phase 3 re-review notes (T008a):** plan sentences now superseded by 12A still read as current — §Amendment's 11A row ("only `update_banter`'s event branch passes a non-zero wait"), §Design tension 13 and §Design tension 3 (venue arrival with no beat), §Design 5 Phase 3's `say(line, Duration::ZERO)`: mark each superseded at the close-out. `EVENT_BEAT_MS`'s doc in `src/lib.rs` names only event lines — add a 12A clause at the close-out. Whether the menu-select sound is under 400 ms (so the venue's first burble truly follows it) is for the ear; optionally extend `the_event_beat_outlasts_the_round_sounds`. ROADMAP must gain the run-over taunt (13A).
