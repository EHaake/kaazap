...
- [ ] **T002** — `scripts/gen_sfx.py` + `assets/sfx/burble.wav` (new) +
  ...
  arm, `pub const BURBLE_PITCHES: [f32; 5]` and `pub fn burble_cue(word: usize)
  -> Cue` with the plan's docs. Tests: `the_burble_is_softer_than_the_music`
  (decode `Sfx::Burble.bytes()` and `MUSIC_BYTES` with `rodio::Decoder` over a
  `Cursor` — no device; the music's RMS over its first 60 s of samples, by the
  decoder's own sample rate and channel count; assert `burble peak <=
  music_rms * d.music_volume / d.sfx_volume` with `d = Settings::default()`;
  `println!` both figures), `a_burble_ends_before_the_next_word`,
  `each_word_has_its_own_burble_pitch`. **Set `peak`** from the measured music
  RMS, comfortably under the ceiling, and say what you chose and why. If the
  60-second decode makes `cargo test -q` noticeably slow (more than a few
  seconds for this test in a debug build), shorten the window to 30 s and say
  so. In `CREDITS.md`, the *Sound effects* sentence names the burble as
  ...
  *Verify: the full command, verbatim, green, no new warnings, the three new
  tests passing by name. Then `cargo test -q
  the_burble_is_softer_than_the_music -- --nocapture 2>&1 | tail -n 15`, its
  printed peak and music RMS quoted in the report. The script's own output
  ...
  report quotes the `BURBLE` block and `burble_cue` verbatim. The orchestrator
  stages `assets/sfx/burble.wav` by path.*

- [ ] **T003** — `src/app.rs` + `src/board.rs` + `src/settings.rs` (doc) +
  `design/brief.md` + `Readme.md`: the speech seam. Per plan §Design 5
  (*Phase 1*), §Design 6, §Design 10 and §Design tensions 1, 3–5. In
  `app.rs`: field `banter` → `speech: Option<Speech>` with the plan's doc;
  `fn say`, `fn line_visible`, `fn advance_speech` with the plan's docs.
  ...
  `last`, and `update_banter` still uses `lines_for` (T006 changes both). In
  `board.rs`: `pub fn shows_banter(&self) -> bool`, with the plan's doc.
  ...
  status-paragraph phrase, verbatim from plan §Design 10. Do not run
  `cargo fmt`.
  ...
  *Verify: the full command, verbatim, green, no new warnings. `grep -nw
  ...
  empty. `grep -n 'burble_cue(' src/app.rs` returns **exactly two** lines, one
  in `say` and one in `advance_speech` (the import line has no `(`).
  ...
  shows exactly `src/app.rs`, `src/board.rs`, `src/settings.rs`,
  `design/brief.md`, `Readme.md`. The report quotes `say`, `line_visible`,
  `advance_speech` and the changed lines of `tick` verbatim. Then the
...
- [ ] **T006** — `src/app.rs`: lines that follow the series. Per plan §Design 5
  ...
  with the plan's doc. `start_match` draws `start_lines(banter_for(opp_id),
  self.series_state_now())` with **`self.banter_last`** as `last` (not
  `None`). `update_banter`'s rematch branch draws `start_lines(…,
  ...
  *Verify: the full command, verbatim, green, no new warnings. `grep -n
  ...
  pins. `git diff --stat` shows exactly `src/app.rs`. No existing assertion
  changes. The report quotes `series_state_now` and the changed lines of
...
- [ ] **T008** — `src/app.rs`: the venue line on arrival. Per plan §Design 5
  ...
  *Verify: the full command, verbatim, green, no new warnings. `grep -n
  ...
  drives the Phase 3 walkthrough (plan §Verification) at both widths on a
  scratch `KAAZAP_DATA_DIR`.*
...
- [ ] **T009** — Close-out. Draft `specs/030-series-banter/closeout-main-docs.md`
  ...
  **ROADMAP**: mark *Series-aware banter* shipped as spec 030 (`grep -n -i
  ...
  elsewhere; a skip key; and, if the person's answer to plan §Open questions 1
  leaves the compact board silent, that as a possible later change.
  **DECISIONS**: the rulings 1B, 2A, 3B, 4A, 5B, 6A and the spec's unruled
  defaults; the person's by-ear approval of the burble (AC 9) and any tweaks
  (the sub-lettered T002 tasks, with the final `BURBLE` numbers); their answer
  to §Open questions 1. Then the plan's design calls with their reasons: one
  `Speech` replacing `banter`; reveal by blanking rather than slicing; burbles
  only from advancing the one `Speech`, at most one per step; the Animations
  ...
  start (the one Quick Play effect); the resume interpretation.
  ...
  **Mechanical checks** (three-dot, since `main` may move): `git diff
  ...
  'burble_cue(' src/app.rs` → exactly two lines. `cargo build --all-targets`
  ...
  *Verify: three green tails, zero failures; every mechanical check listed with
  its command and fresh output; all sixteen criteria ticked with evidence; the
  sweep clean or its findings resolved. `closeout-main-docs.md` is a new,
...
checks that no input path in `handle_key` reads `speech` (AC 11), and that the
two `burble_cue(` sites are the only burble emitters. The Phase 3 review also
...
scripts/gen_sfx.py burble`, run the full command green,
and `git status --porcelain assets/sfx` showing only ` M
assets/sfx/burble.wav`. The person listens again. A tweak is routine, not a
decision review. **If they want it louder than the test's ceiling allows**, that
is plan §Open questions 3: a product question, not a weaker test. Phase 2 may
start once the person says continue, even if the burble is still being tuned.
Tweaks are independent of Phases 2–3, and AC 9 stays unticked until approval.

**Before sign-off, plan §Open questions 1 goes to the person**: on the compact
board, no line shows, so should the opponent still be heard? The draft plays
nothing there (the spec's literal reading). A different answer is a one-line
change in `line_visible` and a sentence in plan §Design tension 5, made before
T003 is dispatched.
