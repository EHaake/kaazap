...
| Post-sweep change: Records Screen→overlay + centered text (Erik call) impl (sdd-implementer) | opus (fable budget short — documented fallback) | ~43K (measured return) | done; Screen::Records removed, Modal::Records popup, block-centered text, 341 tests green |
| Post-sweep tweak: Records popup margin + vertical centering (Erik call) impl (sdd-implementer) | opus (fable budget short — documented fallback) | ~18K (measured return) | done; blank margin row after header + body vertically centered when it fits, 341 tests green; driver-attested at 139×31 (no separate review — cosmetic draw-only) |
| Post-sweep tweak: Records popup content-sized box (Erik call) impl (sdd-implementer) | opus (fable budget short — documented fallback) | ~20K (measured return) | done; box sized to content across views (was fixed 55%×75%), capped to terminal; 341 tests green; driver-attested at 139×31 (all 10 opponents fit, no scroll) and 180×48 (stays snug, doesn't balloon) — cosmetic draw-only, no separate review |
| Post-sweep tweak: Records popup bottom padding (Erik call) impl (sdd-implementer) | opus (fable budget short — documented fallback) | ~19K (measured return) | done; one blank row below the footer (was flush to border), box +1 row; 341 tests green; driver-attested at 139×31 — cosmetic draw-only |
| Post-sweep tweak: Records footer-on-final-row + blank line under "By opponent:" (Erik call) impl (sdd-implementer) | opus (fable budget short — documented fallback) | ~18K (measured return) | done; gap moved above footer (footer on last row), blank line between header and table; 341 tests green; driver-attested at 139×31 — cosmetic draw-only |
| Post-sweep tweak: anchor breakdown table + per-opponent match/round win% (Erik call) impl (sdd-implementer) | opus (fable budget short — documented fallback) | ~25K (measured return) | done; fixed 5th summary row anchors the table across views; each opponent row shows W–L (P%) for matches and rounds; 343 tests green (2 new regression tests); driver-attested at 139×31 |
| Post-sweep tweak: drop "By opponent:" label + bold column header (Erik call) impl (sdd-implementer) | opus (fable budget short — documented fallback) | ~21K (measured return) | done; removed label (+blank), table up 2 rows / box auto-shrinks 2; header row drawn Emphasis::Strong (bold); anchoring test repointed to opponent_header(); 343 tests green; driver-attested layout at 139×31 (bold is a terminal attr, not visible in the text-grid snapshot) |
| Post-sweep change review (skeptical-reviewer) | opus (fable budget short — documented fallback) | ~41K (measured return) | signed off, no blocking; borrows sound, panic-free at all reachable sizes (never drawn below 139×31), Esc preserves menu selection structurally (AC1 holds). Notes accept-and-log: no routing unit test; block_w relies on draw_text_in clipping; odd-padding half-cell cosmetic |
