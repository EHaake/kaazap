...
- [x] **T006** — `src/shop.rs`, `src/portrait.rs`, `src/board.rs`: the
...
| Pre-merge whole-spec sweep (skeptical-reviewer) | opus | ~130K (measured return) | 2 blocking: (1) stale app.rs settle/record comment carried from the T004 review and not closed there; (2) CLAUDE.md still described the pre-021 loop — protocol miss: the constitution should have been amended before implementation. Closed as resolved: stake_match doc, abandoned escrow, banner net-gain rule, cheapest_floor docs claim, AC2 driver-only, T001 additive wording. Still open (no block): banner_line untested; net-gain expression in two files; wager width test never walks to k_max; shop width test duplicates the format string; WagerState::new precondition prose-only; draw emphasis by row index; pre-021 in_progress not pinned via Profile::from_json; forfeit paths driver-only; AC5 rematch records driver-only; start_match doc unverified; portrait.rs:13-16 stale; run-over notice has no fit test; DECISIONS spec-020 completion bullet left as written (pointer for the person) |
| Sweep fix (sdd-implementer) | opus | ~30K (measured return) | app.rs comment rewritten; CLAUDE.md amendment drafted in closeout-main-docs.md §0; applied on main by the orchestrator in its own commit (7081514) and merged into the branch |
| Sweep re-review (skeptical-reviewer) | opus | ~20K (measured return) | signed off, both resolved; notes: orchestrator grep confirmed the completion edge is inside settle_campaign_match; CLAUDE.md Architecture still names "shop/pack-opening" and "pack opening" as future modes — product question for the person; §0 marked applied; wording tidy on main |
